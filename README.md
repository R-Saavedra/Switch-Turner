...
The code is still in the development stage, but we hope that the main script comes out quite soon. Nevertheless we still offer some scripts that (with some manual work) may be helpful to re-parametrize your system's torsions. This includes a calculator of single-points via AMBER and the main fitter script that we are employing (fit_topology.sh and create_topology_files.sh are not necessary to run the fitter, though they do ease up some work), that through the Molecular Mechanics and Reference Energy data generates fitted torsion parameters in the style of AMBER Force Fields.
...
<p align="center">
  <img src="logo-switch-turner.png" alt="Logo" width="400">
</p>
